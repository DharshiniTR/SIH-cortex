      ...
      if (res.ok) {
        ...
        if (_this2.isMaybeSingle && Array.isArray(data)) if (data.length > 1) {
          ...
          if (_this2.shouldThrowOnError) {
            var _error$hint;
            throw new PostgrestError(_objectSpread2(_objectSpread2({}, error2), {}, { hint: (_error$hint = error2.hint) !== null && _error$hint !== void 0 ? _error$hint : "" }));
          }
  ...
  async function handleError2(error2) {
    ...
    try {
      ...
      if (NETWORK_ERROR_CODES.includes(error2.status)) {
        throw new AuthRetryableFetchError(error2.statusText || `HTTP ${error2.status}`, error2.status);
      }
      throw new AuthUnknownError(_getErrorMessage2(e), e);
    }
    if (NETWORK_ERROR_CODES.includes(error2.status)) {
      throw new AuthRetryableFetchError(_getErrorMessage2(data), error2.status);
    }
...
${suffix}`;
  ...
  // src/lib/supabase.ts
  var SUPABASE_URL = "https://esbnkksbtaikqzalcwhe.supabase.co";
  var SUPABASE_PUBLISHABLE_KEY = "sb_publishable_JGX5gZ0BIzWjTkkhucHEQQ_ipWXxCoz";
  var chromeStorage = {
    getItem: async (key) => {
  ...
  async function init() {
    ...
    const pendingDownloads = Array.isArray(stored.pendingDownloads) ? stored.pendingDownloads : [];
  ...
  `;
    ...
    document.querySelector("#upload-form").onsubmit = async (event) => {
      ...
      try {
        ...
        const { data: existingDoc } = await supabase.from("docs").select("*").eq("email", email).eq("name", String(form.get("name"))).maybeSingle();
        if (existingDoc) {
          throw new Error("A document with this name already exists.");
        }
        const fileToDataUrl = (file2) => {
          return new Promise((resolve, reject) => {
